...
class TrainNumpy:
    ...
    @staticmethod
    def _print_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, labels: list[str]) -> None:
        matrix = confusion_matrix(y_true, y_pred, labels=np.arange(len(labels)))
        print("\nFinal confusion matrix (rows=true, cols=pred):")
        header = "           " + " ".join(f"{label[:9]:>9}" for label in labels)
        print(header)
        for label, row in zip(labels, matrix):
            row_str = " ".join(f"{int(value):9d}" for value in row)
            print(f"{label[:9]:>9} {row_str}")
    ...
    def train(
        ...
        tfidf_word_features: int = 5000,
        tfidf_char_features: int = 5000,
    ):
        df = get_datasets()
        ...
        tfidf_word = TFIDF(ngram_range=(1, 2), max_features=tfidf_word_features)
        tfidf_char = TFIDF(ngram_range=(2, 5), max_features=tfidf_char_features)

        X_train_word = tfidf_word.fit_transform(X_train_word_texts)
        X_train_char = tfidf_char.fit_transform(X_train_char_texts)
        ...
        TrainNumpy._print_confusion_matrix(y_eval, eval_preds, unique_labels)
